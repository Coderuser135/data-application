...
export async function getProductById(id) {
  return Product.findOne({ _id: id, is_active: true }).populate('category_id');
}
...
export async function updateProduct(id, data) {
  ...
  const next = {
    name: data.name !== undefined ? String(data.name).trim() : product.name,
    price: data.price !== undefined ? data.price : product.price,
    sale_price: data.sale_price !== undefined ? data.sale_price : product.sale_price,
  ...
  const update = {
    ...
    category_id: data.category_id !== undefined ? data.category_id || null : product.category_id,
    is_active: data.is_active !== undefined ? Boolean(data.is_active) : product.is_active,
  };
...
export async function createCategory(data) {
  ...
  const slug = slugify(data.name);
  return ProductCategory.create({ name: String(data.name).trim(), description: data.description || '', slug });
}
...
export async function updateCategory(id, data) {
  ...
  if (data.name !== undefined) {
    if (!String(data.name).trim()) throw new ApiError(400, 'Category name is required');
    category.name = String(data.name).trim();
