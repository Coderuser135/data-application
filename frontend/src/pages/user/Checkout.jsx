...
export default function Checkout() {
  ...
  function validate() {
    ...
    if (!form.pincode.trim() || form.pincode.length < 6) return 'Valid pincode is required';
    return null;
  }
  ...
  async function handleCheckout(e) {
    ...
    try {
      ...
      try {
        ...
        if (razorpayOrder && keyId && window.Razorpay) {
        ...
        // Payment failed or cancelled — order is still created with pending payment
      ...
      setError(typeof err === 'string' ? err : 'Checkout failed. Please try again.');
    } finally {
      setLoading(false);
    }
  }
  ...
  if (success) return (
    <div className="flex flex-col items-center justify-center py-20 text-center">
      ...
      <h2 className="mt-4 font-display text-2xl font-bold text-ink-950 dark:text-white">Order placed!</h2>
      <p className="mt-2 text-surface-500">Redirecting to your orders...</p>
    </div>
  );
        ...
        <Card>
          ...
          <form onSubmit={handleCheckout} className="space-y-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Input label="Phone" value={form.phone} onChange={(e) => update('phone', e.target.value)} placeholder="10-digit phone" maxLength={10} required />
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              ...
              <Input label="Pincode" value={form.pincode} onChange={(e) => update('pincode', e.target.value)} placeholder="6-digit PIN" maxLength={6} required />
            </div>
            <div className="rounded-xl bg-surface-50 p-4 dark:bg-surface-800">
              ...
              <p className="mt-1 text-xs text-surface-500">Online payment via Razorpay. Your payment is verified server-side for security.</p>
            </div>
