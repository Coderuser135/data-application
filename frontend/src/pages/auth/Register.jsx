...
export default function Register() {
  ...
  async function handleSubmit(e) {
    ...
    setError('');
    try {
      await dispatch(signUp({ email: form.email, password: form.password, fullName: form.fullName, phone: form.phone })).unwrap();
  ...
  return (
    <div className="flex min-h-screen">
      <div className="relative hidden w-1/2 overflow-hidden bg-ink-950 lg:block">
        ...
        <div className="absolute -right-20 bottom-1/4 h-96 w-96 rounded-full bg-brand-500/20 blur-[100px]" />
        <div className="relative flex h-full flex-col justify-between p-12 text-white">
          ...
          <div>
            <p className="font-display text-4xl font-bold leading-tight">Start your<br /><span className="text-brand-400">strongest chapter.</span></p>
            <ul className="mt-8 space-y-4">
              {benefits.map((b) => (
                <li key={b} className="flex items-center gap-3 text-surface-300">
                  <span className="flex h-6 w-6 items-center justify-center rounded-full bg-brand-500/20 text-brand-400"><Check size={14} /></span>
                  {b}
                </li>
              ))}
            </ul>
      ...
      <div className="flex w-full items-center justify-center bg-surface-50 px-5 py-12 dark:bg-surface-950 lg:w-1/2">
        <div className="w-full max-w-md">
          ...
          <h1 className="font-display text-3xl font-bold text-ink-950 dark:text-white">Create your account</h1>
          <p className="mt-2 text-sm text-surface-500">Join IronForge and start training with purpose</p>
          ...
          <form onSubmit={handleSubmit} className="mt-7 space-y-5">
            <Input label="Full name" value={form.fullName} onChange={(e) => update('fullName', e.target.value)} placeholder="e.g. Rahul Sharma" required />
            ...
            <Input label="Password" type="password" value={form.password} onChange={(e) => update('password', e.target.value)} placeholder="At least 6 characters" required />
            ...
            <Button type="submit" loading={loading} className="w-full" size="lg">Create account <ArrowRight size={16} /></Button>
          </form>
          <p className="mt-7 text-center text-sm text-surface-500">Already have an account? <Link to="/login" className="font-semibold text-brand-600 hover:text-brand-700">Sign in</Link></p>
        </div>
      </div>
    </div>
  );
